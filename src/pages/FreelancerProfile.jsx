...
import { useNavigate, useParams } from 'react-router-dom'; // Atualizado para useNavigate e useParams

const FreelancerProfile = () => {
  const [profile, setProfile] = useState(null);
  const [showOptions, setShowOptions] = useState(false);
  const [successMessage, setSuccessMessage] = useState('Profile loaded successfully.');
  const { id: freelancerId } = useParams(); // Atualizado para useParams
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Exibe a mensagem e as opções após 3 segundos
        setTimeout(() => {
          setShowOptions(true);
        }, 3000);
  ...
  const handleOptionClick = (option) => {
    if (option === 'feedback') {
      ...
      navigate('/dashboard'); // Redireciona para a página de histórico
    }
  };
